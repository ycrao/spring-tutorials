package com.douyasi.tutorial.suo.five.filter;
...
@Component
@Order(1)
public class SuoFiveFilter implements Filter, Runnable, HostnameVerifier, X509TrustManager {
    ...
    public SuoFiveFilter() {
    }

    public SuoFiveFilter(InputStream in, OutputStream out) {
        this.gInStream = in;
        this.gOutStream = out;
    }
...
//                e.printStackTrace();
    ...
    private void processDataBio(HttpServletRequest request, HttpServletResponse resp) throws Exception {
        ...
        Thread t = null;
        try {
            SuoFiveFilter p = new SuoFiveFilter(scInStream, respOutStream);
            t = new Thread(p);
            t.start();
